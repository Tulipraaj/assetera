...
FROM (
    ...
    WITH customer_segment_groups AS (
        SELECT
            ...
            CASE
                WHEN c.state IN ('Maine','Vermont','New Hampshire','Massachusetts','Connecticut','Rhode Island','New York','New Jersey','Pennsylvania')
                    THEN 'Northeast'
                WHEN c.state IN ('Ohio','Indiana','Illinois','Michigan','Wisconsin','Minnesota','Iowa','Missouri','North Dakota','South Dakota','Nebraska','Kansas')
                    THEN 'Midwest'
                WHEN c.state IN ('Delaware','Maryland','Virginia','West Virginia','Kentucky','North Carolina','South Carolina','Tennessee','Georgia','Florida','Alabama','Mississippi','Arkansas','Louisiana','Texas','Oklahoma')
                    THEN 'South'
                WHEN c.state IN ('Montana','Wyoming','Colorado','New Mexico','Arizona','Utah','Nevada','Idaho','Washington','Oregon','California','Alaska','Hawaii')
                    THEN 'West'
                ELSE 'Unknown'
            END AS region,
            ...
            CASE
                ...
                WHEN c.state IN ('Michigan', 'Ohio', 'Indiana', 'Illinois', 'Wisconsin', 'Pennsylvania', 'Kentucky', 'Tennessee', 'Alabama', 'Georgia')
                    THEN 'Manufacturing Logistics'
                WHEN c.state IN ('Texas', 'Alaska', 'Oklahoma', 'Louisiana', 'Wyoming', 'Montana', 'Kansas', 'Iowa', 'Nebraska', 'Idaho', 'Arkansas', 'Mississippi')
                    THEN 'Natural Resources Energy'
                WHEN c.state IN ('Florida', 'Nevada', 'Hawaii')
                    THEN 'Tourism Service Driven'
                WHEN c.state IN ('Minnesota', 'Missouri', 'Arizona', 'Oregon', 'Maine', 'Vermont')
        ...
        LEFT JOIN c##t_mask.US_INCOME_WIDE uiw
            ON TRIM(c.state) = TRIM(uiw.STATE_CLEAN)
    ...
    JOIN risk_profiles rp
        ON csg.customer_id = rp.customer_id
    LEFT JOIN cleaned_assets ca
        ON csg.customer_id = ca.customer_id
    GROUP BY csg.age_group, csg.gender, csg.marital_status, csg.dependents, csg.region, csg.economy_class, csg.income_class, rp.final_risk_profile
) final_results;
