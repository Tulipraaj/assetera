...
def fetch_prices(tickers: List[str], start: date, end: date) -> pd.DataFrame:
    ...
    yf_end = end + timedelta(days=1)  # inclusive window
    raw = yf.download(
        tickers=tickers, start=start, end=yf_end,
        progress=False, auto_adjust=True, group_by="ticker", threads=True,
    )
    if raw is None or len(raw) == 0:
        return pd.DataFrame()
    field = _pick_price_field(raw)
    if len(tickers) == 1:
        df = raw[[field]].rename(columns={field: tickers[0]})
    else:
        if isinstance(raw.columns, pd.MultiIndex):
            frames = [raw[(t, field)].rename(t) for t in tickers if (t, field) in raw.columns]
            df = pd.concat(frames, axis=1) if frames else pd.DataFrame()
        else:
            present = [c for c in raw.columns if c in tickers]
            df = raw[present].copy()
    if df.empty:
        return df
    df.index.name = "Date"
...
st.sidebar.caption("End date can be **any day up to today**. Prices via yfinance (adjusted).")
